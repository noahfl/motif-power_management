...
"""
Tests wget runs at each processor step; runs
...
def trace_runs(file, freq):
    ...
    wget download, assigns process to core 0, and calls
    ...
    iterator = 0.1
    min_freq = 0.8
    link = ('https://www.python.org/ftp/python/3.4.1/Python-3.4.1.tar.xz')
    while (frequency >= min_freq):
        #rounded version of frequency
        frequency = round(frequency, 1)
        freq_str = str(round(frequency, 1))
        ...
        for i in range(run_range):
            ...
            # == 'wget -P ./download_files [link]'
            proc = Popen(['sudo', 'wget', '-P', './download_files/', link])
            pid = str(proc.pid)
            ...
            print(set_core.pid)
##            filename = ('./cpu_output_files/strace_' + freq_str + '_run_' +
##                        str(i) + '.txt')
##            # == 'strace -o [file] -p [pid]'
##            strace = Popen(['sudo', 'strace', '-o', filename, '-p', pid])
        ...
        frequency -= iterator
        del_dls = Popen(['sudo', 'rm', './downloaded_files/*'])
        del_dls.communicate()
